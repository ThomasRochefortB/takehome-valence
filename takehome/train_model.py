...
import safe  # Assuming this is the correct import for the safe library
from safe.tokenizer import SAFETokenizer  # Import and set up the SAFETokenizer
...
def generate_smiles_embedding(smiles):
    ...
    try:
        ...
        safe_str = safe_tokenizer.eos_token  # This should be the EOS token from the tokenizer
    
    # Encode the SMILES or the fallback string using the tokenizer
    tokens = safe_tokenizer.encode(safe_str)
    tokens_tensor = torch.tensor([tokens]).to(device)  # Convert to tensor, add batch dimension, and move to device

    # with torch.no_grad():
    #     outputs = hf_model(tokens_tensor)

    # # Extract the embeddings, move them back to the CPU, and convert to NumPy array
    # last_hidden_state = outputs.last_hidden_state  # (batch_size, sequence_length, hidden_size)
    # embeddings = last_hidden_state.mean(dim=1).squeeze().cpu().numpy()  # Move to CPU and convert to NumPy array
    ...
    with torch.no_grad():
        ...
        positional_embeddings = transformer.wpe(torch.arange(tokens_tensor.size(1), device=device).unsqueeze(0))
    ...
    combined_embeddings = combined_embeddings.mean(dim=1).squeeze().cpu().numpy()  # (sequence_length, embedding_size)
...
base_models = {
    ...
    'lgbm': lgb.LGBMRegressor(n_estimators=500, max_depth=10, min_data_in_leaf=25,feature_fraction=0.3, random_state=42, verbose=-1),
    'svr': SVR(C=10.0, epsilon=0.1),
    'mlp': MLPRegressor(hidden_layer_sizes=(256,256,256),alpha=1.0,early_stopping=True, max_iter=200, random_state=42)  # Add MLPRegressor
}
...
def get_dataset(config):
    ...
    if X.size > 0:
        ...
        highly_correlated_features = correlation_with_target[correlation_with_target.abs() > high_correlation_threshold]

        if not highly_correlated_features.empty:
            print("Warning: The following features are highly correlated with the target:")
...
def train_and_evaluate_model(X, y, config, model, model_name, log_file):
    ...
    # Print train and test RMSE for each run
    print(f"\n{model_name} - Mean Train RMSE: {train_rmse_scores.mean():.4f}, Mean Test RMSE: {rmse_scores.mean():.4f}")
